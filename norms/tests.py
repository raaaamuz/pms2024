...
class Percentile(APIView):
    ...
    def get(self, request):
        if request.user.is_authenticated:
            ...
            logger.debug(f"top data: {top.head()}")
            ...
            return Response({"Headers": header, "Top Box": top_percentile, "Top 2 Box": top2_percentile, "Mean Value": mean_value, "Mean": mean_percntile, "Samples considered": sample, "Number of products": nop, "Number of projects": tot_project})
        else:
